...
.select2-container--default .select2-selection--single .select2-selection__arrow {
    ...
    <div class="main-content">
        ...
        <tr>
          ...
          <td><input type="text" name="TCN" id="TCN_{{ $row->id }}" class="form-control form-control-sm w-100" value="{{ $row->TCN }}"></td>
          
          <td><input type="text" name="HCN_NO" id="HCN_NO_{{ $row->id }}" class="form-control form-control-sm w-100" value="{{ $row->TransportBrnCode }}"></td>
...
<script>
    ...
    function updateHCN(id) {
        ...
        var TransportBrnCode = $('#HCN_NO_' + id).val();
        var TCN = $('#TCN_' + id).val();
         
        $.ajax({
            url: '{{ route('update.transport.brn') }}',
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
                id: id,
                 TransportBrnCode: TransportBrnCode,
                 TCN: TCN
            },
